...
void AudioEffectTranscribeInstance::_process(const void *p_src_frames, AudioFrame *p_dst_frames, int p_frame_count) {
	//multiply volume interpolating to avoid clicks if this changes
	const AudioFrame* frames = static_cast<const AudioFrame*>(p_src_frames);
	if (!is_transcribing) {
		for (int i = 0; i < p_frame_count; i++) {
			p_dst_frames[i] = frames[i];
		}
		return;
	}

	//Add incoming audio frames to the IO ring buffer
	const AudioFrame *src = frames;
...
bool AudioEffectTranscribeInstance::_process_silence() const {
	return true;
}

void AudioEffectTranscribeInstance::_update_buffer() {
...
void AudioEffectTranscribeInstance::_io_thread_process() {
...
void AudioEffectTranscribeInstance::_io_store_buffer() {
...
void AudioEffectTranscribeInstance::_thread_callback(void *_instance) {
	AudioEffectTranscribeInstance *aeri = reinterpret_cast<AudioEffectTranscribeInstance *>(_instance);

	aeri->_io_thread_process();
}

void AudioEffectTranscribeInstance::init() {
	...
	io_thread.start(Callable(this, "_thread_callback"));
}

void AudioEffectTranscribeInstance::finish() {
...
Ref<AudioEffectInstance> AudioEffectTranscribe::_instantiate() {
	Ref<AudioEffectTranscribeInstance> ins;
	ins.instantiate();
	ins->is_transcribing = false;

	//Re-using the buffer size calculations from audio_effect_delay.cpp
	float ring_buffer_max_size = AudioEffectTranscribe::IO_BUFFER_SIZE_MS;
	...
	if (current_instance != nullptr) {
		is_currently_recording = current_instance->is_transcribing;
	}
...
void AudioEffectTranscribe::ensure_thread_stopped() {
	if (current_instance != nullptr) {
		current_instance->finish();
	}
}

void AudioEffectTranscribe::set_transcribing_active(bool p_transcribe) {
	if (p_transcribe) {
		if (current_instance == nullptr) {
			WARN_PRINT("Transcribing should not be set as active before Godot has initialized.");
			return;
		}
		ensure_thread_stopped();
		...
		if (current_instance != nullptr) {
			current_instance->is_transcribing = false;
		}
	}
}

bool AudioEffectTranscribe::is_transcribing_active() const {
	if (current_instance == nullptr) {
		return false;
	} else {
		return current_instance->is_transcribing;
	}
}

String AudioEffectTranscribe::get_text() const {
	ERR_FAIL_COND_V(current_instance.is_null(), "");
	return current_instance->text;
}

void AudioEffectTranscribe::_bind_methods() {
	ClassDB::bind_method(D_METHOD("get_text"), &AudioEffectTranscribe::get_text);
	ClassDB::bind_method(D_METHOD("set_transcribing_active", "transcribe"), &AudioEffectTranscribe::set_transcribing_active);
	ClassDB::bind_method(D_METHOD("is_transcribing_active"), &AudioEffectTranscribe::is_transcribing_active);
}
AudioEffectTranscribe::AudioEffectTranscribe() {
